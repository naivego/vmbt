...
def getasline(socna, mdi, mp, mdp, ak, sk_time, atr, i, eti = None):
    ...
    asline.extp = bp
    asline.initi = i
    asline.et_ini = eti if eti else i
    asline.bki = bi
    asline.bkp = bp
    asline.bkcn = 0
    asline.rsta = 1
...
def getmrline1(socna, mri, mrp, ak, sk_time, atr, i, eti = None):
    ...
    mrline.extp = bp
    mrline.initi = i
    mrline.et_ini = eti if eti else i
    mrline.bkcn = 0
    mrline.rsta = 0
    return mrline
# --------------------------------------------------------------------------------------------
...
def getsaline2(socna, bi, bp, sad2, rkp, fbi, sk_time, atr, i, eti = None):
    ...
    saline.extp = ep
    saline.initi = i
    saline.et_ini = eti if eti else i
    saline.bkcn = 0
    saline.rsta = 0
    return saline
...
class Grst_Factor(object):
    def __init__(self, ctaEngine, var, period, skdata, sdt=True, fid= 'ma', teda = None):
        if period == 'd':
    ...
    # ----------------------
    def new_supline(self, fidna, bdpt, edpt, sk_open, sk_high, sk_low, sk_close, sk_volume, sk_time, sk_atr, sk_ckl, i, eti = None):
    ...
    def new_resline(self, fidna, bdpt, edpt, sk_open, sk_high, sk_low, sk_close, sk_volume, sk_time, sk_atr, sk_ckl, i, eti = None):
                    ...
                    if newresl:
                        ...
                        if self.faset['mdl']:
                            newtbl = self.new_resline3(self.fid + '_mdl', self.sk_open, self.sk_high, self.sk_low, self.sk_close, self.sk_volume,
                                                       self.sk_time,
                                                       self.sk_atr, self.sk_ckl, i, self.teix[-1])
                            if newtbl:
            ...
            if None and ydbblrsta % 3 == 2 and tdbblrsta % 3 == 0 and tdbblrsta != ydbblrsta:  # None and
                if len(self.botms) > 1 and self.ctp:
                    ...
                    self.new_supline(self.botms[-2], self.ctp, self.sk_open, self.sk_high, self.sk_low, self.sk_close, self.sk_volume,
                                     self.sk_time, self.sk_atr, self.sk_ckl, i)
                    if self.crtbbl:
    ...
    def colfas(self, flg=None):
        ...
        self.quotes['zsh' + aflg] = self.sk_zsh
        self.quotes['zsl' + aflg] = self.sk_zsl
        self.quotes['qswr' + aflg] = self.sk_qswr
        ...
        # self.quotes['alp3'+aflg] = self.sk_alp3
        # self.quotes['alp4'+aflg] = self.sk_alp4
        # self.quotes['alp5'+aflg] = self.sk_alp5
        # self.quotes['alp6'+aflg] = self.sk_alp6
        self.quotes['dlp1' + aflg] = self.sk_dlp1
        self.quotes['dlp2' + aflg] = self.sk_dlp2
        # self.quotes['dlp3'+aflg] = self.sk_dlp3
        # self.quotes['dlp4'+aflg] = self.sk_dlp4
        # self.quotes['dlp5'+aflg] = self.sk_dlp5
        # self.quotes['dlp6'+aflg] = self.sk_dlp6

        self.quotes['rstn' + aflg] = self.sk_rstn
        self.quotes['rstspl' + aflg] = self.sk_rstspl
        self.quotes['rstsph' + aflg] = self.sk_rstsph

        self.quotes['bsl' + aflg] = self.sk_bsl
        self.quotes['bsh' + aflg] = self.sk_bsh

        self.quotes['sk_zs1c' + aflg] = self.sk_zs1c
        self.quotes['sk_zs1a' + aflg] = self.sk_zs1a
        self.quotes['sk_zs2c' + aflg] = self.sk_zs2c
        self.quotes['sk_zs2a' + aflg] = self.sk_zs2a
        self.quotes['sk_qs1c' + aflg] = self.sk_qs1c
        self.quotes['sk_qs1a' + aflg] = self.sk_qs1a
        self.quotes['sk_qs2c' + aflg] = self.sk_qs2c
        self.quotes['sk_qs2a' + aflg] = self.sk_qs2a
