...
class BacktestingEngine(object):
    ...
    def __init__(self,  bt_Config, eventEngine = None):
        ...
        self.DB_Rt_Dir = bt_Config['DB_Rt_Dir']
        ...
        self.initCapital = bt_Config['InitCapital']           # 期初资金
    ...
    def loadDataFromMongo(self, vadas):
        ...
        for vdn, vada in vadas.iteritems():
            if not vada:
                continue
            print 'Load' + vdn+ ' for ' + vada.var + '_'+ vada.dmn
            collection = self.dbClient[vada.dmn][vada.var]
    ...
    # ----------------------------------------------------------------------
    def sendOrder(self, vtSymbol, orderType, price, volume, strategy, ocogrpid = ''):
        """发单"""
    ...
    def cancelOrders(self, symbol, offset=EMPTY_STRING):
        ...
        for vtOrderID in self.workingLimitOrderDict.keys():
            ...
            if order.vtSymbol == symbol and offsetCond:
    ...
    #----------------------------------------------------------------------
    def sendStopOrder(self, vtSymbol, orderType, price, volume, strategy, ocogrpid = ''):
        """发停止单（本地实现）"""
    ...
    #----------------------------------------------------------------------
    def cancelLsOrder(self, vtOrderID):
        """撤单"""
        # 如果发单号为空字符串，则不进行后续操作
    ...
    # ----------------------------------------------------------------------
    def addocoOrder(self, grpID, orderID):
        if not orderID:
            return
        if grpID not in self.ocoListDict:
            self.ocoListDict[grpID] = [orderID]
        elif orderID not in self.ocoListDict[grpID]:
            self.ocoListDict[grpID].append(orderID)
    # ----------------------------------------------------------------------
    def getocoOrderids(self, grpID):
        if not grpID or grpID == '':
            return []
        if grpID in self.ocoListDict:
            return self.ocoListDict[grpID]
        else:
            return []
    # ----------------------------------------------------------------------
    def clearocoOrder(self, grpID = None):
        if not grpID or grpID == '':
            self.ocoListDict.clear()
        elif grpID in self.ocoListDict:
            del self.ocoListDict[grpID]
    ...
    def crossLimitOrder(self):
        ...
        for orderID in self.workingLimitOrderDict.keys():
            if orderID not in self.workingLimitOrderDict:
                continue
            ...
            if buyCross or sellCross:
                ...
                grpid = order.ocogrpid
                todelids =  self.getocoOrderids(grpid)
                for orderID in todelids:
                    self.cancelLsOrder(orderID)
    ...
    def crossStopOrder(self):
        ...
        for stopOrderID in self.workingStopOrderDict.keys():
            if stopOrderID not in self.workingStopOrderDict:
                continue
            so = self.workingStopOrderDict[stopOrderID]
            # 判断是否会成交
